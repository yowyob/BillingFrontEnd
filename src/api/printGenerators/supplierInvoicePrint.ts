// Mirrors facturePrint.ts, but flipped: the supplier is the issuer (header/
// brand block) and our organization is the recipient (bill-to block), since
// this is an invoice the supplier sent to us, not one we sent to a client.
...
<head>
    <meta charset="UTF-8">
    <title>Supplier Invoice - ${data.numeroFacture}</title>
    <style>
        ...
        * { box-sizing: border-box; -webkit-print-color-adjust: exact; }
        ...
        .balance-due { background-color: #fef2f2; color: #dc2626; }
    ...
    <div class="a4-page">
        <header class="header">
            <div class="brand-section">
                <div class="logo-placeholder">${data.nomFournisseur?.charAt(0) || 'S'}</div>
                <div class="doc-title">
            ...
            <div class="seller-info">
                <h2 class="seller-name">${data.nomFournisseur ?? ''}</h2>
                <div class="contact-details">
                    <p>${data.adresseFournisseur ?? ''}</p>
                    <p style="color: #2563eb;">${data.emailFournisseur ?? ''}</p>
                </div>
            </div>
        </header>
        ...
        <section class="info-grid">
            <div>
                ...
                <p class="client-name">${org.organizationName ?? ''}</p>
                <p class="client-address">${org.agencyAddress ?? ''}${org.agencyAddress && org.agencyCity ? ', ' : ''}${org.agencyCity ?? ''}</p>
            </div>
            <div class="date-box">
                <div class="date-row">
                    <span class="date-label">Invoice Date</span>
                    <span class="date-value">${formatDate(data.dateFacture)}</span>
                </div>
                <div class="date-row">
                    <span class="date-label">Due Date</span>
                    <span class="date-value" style="color: #2563eb;">${formatDate(data.dateEcheance)}</span>
                </div>
            </div>
        </section>
        ...
        <footer class="footer">
            <div class="terms">
                ...
                <div class="terms-content">
                    ...
                    <p class="terms-text">Payment is due by the date specified above, per our agreed purchasing terms.</p>
                </div>
            </div>
            ...
            <div class="totals-box">
                ...
                <div class="balance-row ${balanceDue === 0 ? 'balance-paid' : 'balance-due'}">
