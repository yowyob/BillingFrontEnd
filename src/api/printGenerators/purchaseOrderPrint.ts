// Mirrors quotationPrint.ts's exact A4 layout/CSS, adapted for the raw
// BonAchatResponse shape (supplierName/lines/grandTotal, not the admin
// page's mapped PurchaseOrderResponse UI model).
...
export const generatePurchaseOrderHTML = (
  ...
  const tableRows = (data.lines ?? []).map((item: any) => `
    <tr>
        ...
        <td class="text-right total-col">${formatCurrency(item.totalAmount)}</td>
    </tr>
  `).join('') || '';
...
<head>
    <meta charset="UTF-8">
    <title>Purchase Order - ${data.numeroBonAchat}</title>
    <style>
    ...
    <div class="a4-page">
        <header class="header">
            <div class="brand-section">
                ...
                <div class="doc-title">
                    ...
                    <p class="doc-ref">${data.numeroBonAchat}</p>
                </div>
            </div>
        ...
        <section class="info-grid">
            <div>
                ...
                <p class="client-name">${data.supplierName}</p>
                <p class="client-address">${data.supplierAddress || 'Standard Supplier'}</p>
            </div>
            <div class="date-box">
                <div class="date-row">
                    <span class="date-label">Order Date</span>
                    <span class="date-value">${formatDate(data.dateBonAchat)}</span>
                </div>
                <div class="date-row">
        ...
        <footer class="footer">
            <div class="terms">
                ...
                <div class="terms-content">
                    ...
                    <p class="terms-text">This purchase order is subject to the standard terms agreed with your organization. Digital approval via the portal is legally binding.</p>
                </div>
            </div>
            ...
            <div class="totals-box">
                ...
                <div class="grand-total-card">
                    ...
                    <div class="total-amount">${new Intl.NumberFormat('en-GB').format(data.grandTotal ?? 0)}</div>
                </div>
            </div>
        </footer>
    </div>
