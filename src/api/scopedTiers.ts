...
import { getClientsOfflineFirst, getFournisseursOfflineFirst } from "@/src/offline/services/referenceService";
...
export const getVisibleClients = async (): Promise<ClientResponse[]> => {
  ...
  return getClientsOfflineFirst(fetcher);
};
...
export const getVisibleFournisseurs = async (): Promise<FournisseurResponse[]> => {
  ...
  return getFournisseursOfflineFirst(fetcher);
};
