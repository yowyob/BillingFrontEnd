...
// NoteCreditResponse (credit note) fields.
...
export const generateNoteCreditHTML = (
  data: any,
  org: OrgBranding,
  qrBase64: string
): string => {
  ...
  const tableRows = (data.lignesNoteCredit ?? []).map((item: any) => `
    <tr>
        <td class="desc-col">${item.description || item.nomProduit}</td>
        <td class="text-center">${item.quantite}</td>
        <td class="text-right">${formatCurrency(item.prixUnitaire)}</td>
        <td class="text-right total-col">${formatCurrency(item.montantTotal ?? item.credit)}</td>
    </tr>
  `).join('') || '';
...
<head>
    <meta charset="UTF-8">
    <title>Credit Note - ${data.numeroNoteCredit}</title>
    <style>
    ...
    <div class="a4-page">
        <header class="header">
            <div class="brand-section">
                ...
                <div class="doc-title">
                    <h1>Credit Note</h1>
                    <p class="doc-ref">${data.numeroNoteCredit}</p>
                </div>
            </div>
        ...
        <section class="info-grid">
            ...
            <div class="date-box">
                <div class="date-row">
                    ...
                    <span class="date-value">${formatDate(data.dateEmission)}</span>
                </div>
                <div class="date-row">
                    <span class="date-label">Original Invoice</span>
                    <span class="date-value" style="color: #2563eb;">${data.numeroFactureOrigine || '---'}</span>
                </div>
            </div>
        </section>
        ...
        <footer class="footer">
            <div class="terms">
                ...
                <div class="terms-content">
                    <p class="section-label" style="border: none; padding: 0;">Reason</p>
                    <p class="terms-text">${data.motif || 'This credit note offsets the referenced invoice per the terms agreed with your organization.'}</p>
                </div>
            </div>
            ...
            <div class="totals-box">
                ...
                <div class="balance-row balance-paid">
                    <span>${data.statut === 'REMBOURSÉ' ? 'Refunded' : 'Applied As Credit'}</span>
                    <span></span>
                </div>
            </div>
        </footer>
