...
import { BonCommandeResponse, BonCommandeService, AgenciesService, SellerAdminService, KernelAgencyResponse, SellerListItemResponse } from '@/src/src2/api';
...
const SaleOrderJournalPage = () => {
  const [data, setData] = useState<BonCommandeResponse[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      BonCommandeService.getByOrganizationId6(seller.organizationId),
      ...
      .catch(() => toast.error("Failed to load sale order journal data."))
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      const matchesSearch = !filters.search || (item.numeroCommande ?? '').toLowerCase().includes(filters.search.toLowerCase());
      ...
      const itemDate = new Date(item.dateCommande ?? 0);
  ...
  const clearAllFilters = () => setFilters({
    search: "", status: "ALL", agencies: [], client: "", startDate: "", endDate: "", minAmount: "", maxAmount: "",
  });

  const handleExportCsv = () => {
    exportRowsToCsv(
      `sale-order-journal-${new Date().toISOString().slice(0, 10)}`,
      [
        { header: "Order Number", accessor: (r: BonCommandeResponse) => r.numeroCommande },
        { header: "Client", accessor: (r: BonCommandeResponse) => r.nomClient },
        { header: "Agency", accessor: (r: BonCommandeResponse) => (r.agencyId ? agencyById.get(r.agencyId)?.name : undefined) },
        { header: "Date", accessor: (r: BonCommandeResponse) => r.dateCommande },
        { header: "Status", accessor: (r: BonCommandeResponse) => r.statut },
        { header: "Total (TTC)", accessor: (r: BonCommandeResponse) => r.montantTTC },
        { header: "Currency", accessor: (r: BonCommandeResponse) => r.devise },
      ],
      filteredData
    );
  };

  const hasActiveFilters = filters.agencies.length > 0 || filters.search !== "" || filters.client !== ""
    || filters.startDate !== "" || filters.endDate !== "" || filters.minAmount !== "" || filters.maxAmount !== "" || filters.status !== "ALL";

  const getStatusColor = (statut?: BonCommandeResponse.statut) => {
    switch (statut) {
      case BonCommandeResponse.statut.LIVRE:
      case BonCommandeResponse.statut.RECU: return "bg-emerald-50 text-emerald-600 border-emerald-200";
      case BonCommandeResponse.statut.BROUILLON: return "bg-secondary-super-light text-secondary-mid border-secondary-light";
      case BonCommandeResponse.statut.REJETE:
      case BonCommandeResponse.statut.ANNULE: return "bg-red-50 text-red-600 border-red-200";
      case BonCommandeResponse.statut.RECU_PARTIEL: return "bg-amber-50 text-amber-600 border-amber-200";
    ...
    <div className="p-8 bg-secondary-background min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <h1 className="text-3xl font-black text-primary tracking-tight">Sale Order Journal</h1>
          <p className="text-secondary-gray text-sm font-medium">Monitoring organizational sales order lifecycle across agencies.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <div className="relative">
            <select className="w-full px-4 py-2.5 bg-secondary-super-light border border-secondary-light/50 rounded-xl text-sm font-black text-secondary-mid appearance-none text-center cursor-pointer hover:bg-secondary-mid hover:text-white transition-all outline-none" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}>
              ...
              {Object.values(BonCommandeResponse.statut).map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        </div>
              ...
              {loading ? (
                ...
                <tr><td colSpan={5} className="p-24 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-sm font-black text-secondary-gray uppercase tracking-widest">No sale orders found</span>
                  </div>
                </td></tr>
                ...
                return (
                  <tr key={item.idBonCommande} className="hover:bg-secondary-super-light/40 transition-all group relative border-l-4 border-l-transparent hover:border-l-secondary-mid">
                    <td className="px-8 py-5">
                      <div className="flex flex-col">
                        <span className="font-black text-primary group-hover:text-secondary-mid transition-colors">{item.numeroCommande}</span>
                        <span className="text-[11px] font-bold text-secondary-gray flex items-center gap-1 mt-0.5"><Calendar size={10} /> {new Date(item.dateCommande ?? 0).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                      </div>
                    </td>
                    <td className="px-8 py-5">
...
export default SaleOrderJournalPage;
