// Mirrors purchaseOrderPrint.ts's exact A4 layout/CSS, adapted for
// BondeReceptionResponse (goods receipt note) fields. Like purchaseOrderPrint,
// the supplier is the counterparty (not a client), and LineBonReception
// tracks ordered/received/accepted/rejected quantities plus rate/lineAmount.
...
export const generateBondeReceptionHTML = (
  data: any,
  org: OrgBranding,
  qrBase64: string
): string => {
  ...
  const tableRows = (data.lines ?? []).map((item: any) => `
    <tr>
        ...
        <td class="text-center">${item.orderedQuantity ?? 0}</td>
        <td class="text-center">${item.receivedQuantity ?? 0}</td>
        <td class="text-center">${item.acceptedQuantity ?? 0}</td>
        <td class="text-right total-col">${formatCurrency(item.lineAmount)}</td>
    </tr>
  `).join('') || '';

  const totalAmount = (data.lines ?? []).reduce((sum: number, item: any) => sum + (item.lineAmount ?? 0), 0);
...
<head>
    <meta charset="UTF-8">
    <title>Goods Receipt Note - ${data.numeroReception}</title>
    <style>
    ...
    <div class="a4-page">
        <header class="header">
            <div class="brand-section">
                ...
                <div class="doc-title">
                    ...
                    <p class="doc-ref">${data.numeroReception}</p>
                </div>
            </div>
        ...
        <section class="info-grid">
            <div>
                ...
                <p class="client-name">${data.nomFournisseur}</p>
                <p class="client-address">${data.agenceDeTransport || ''}</p>
            </div>
            <div class="date-box">
                <div class="date-row">
                    <span class="date-label">Reception Date</span>
                    <span class="date-value">${formatDate(data.dateReception)}</span>
                </div>
                <div class="date-row">
                    <span class="date-label">Purchase Order Ref</span>
                    <span class="date-value" style="color: #2563eb;">${data.numeroBonAchat || '---'}</span>
                </div>
            </div>
        </section>
        ...
        <footer class="footer">
            <div class="terms">
                ...
                <div class="terms-content">
                    ...
                    <p class="terms-text">${data.notes || 'Goods have been inspected upon reception. Any rejected or damaged quantities are noted per line.'}</p>
                </div>
            </div>
            ...
            <div class="totals-box">
                ...
                <div class="grand-total-card">
                    ...
                    <div class="total-amount">${new Intl.NumberFormat('en-GB').format(totalAmount)}</div>
                </div>
            </div>
        </footer>
    </div>
