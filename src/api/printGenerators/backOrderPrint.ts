...
// BackOrderResponse fields. LigneBackOrder has no unit price total column
// in the invoice sense (no montantTotal on Qty*Price columns) -- it tracks
// quantiteCommandee/quantiteRecue/quantiteEnAttente, so the line items
// table shows those quantities instead of a currency Qty/Unit/Total layout;
// montantTotal (if present) is still shown as a value column.
...
export const generateBackOrderHTML = (
  data: any,
  org: OrgBranding,
  qrBase64: string
): string => {
  ...
  const tableRows = (data.lignes ?? []).map((item: any) => `
    <tr>
        <td class="desc-col">${item.nomProduit || item.referenceProduit}</td>
        <td class="text-center">${item.quantiteCommandee ?? 0}</td>
        <td class="text-center">${item.quantiteRecue ?? 0}</td>
        <td class="text-right total-col">${item.quantiteEnAttente ?? 0}</td>
    </tr>
  `).join('') || '';

  const totalPending = (data.lignes ?? []).reduce((sum: number, item: any) => sum + (item.quantiteEnAttente ?? 0), 0);
...
<head>
    <meta charset="UTF-8">
    <title>Back Order - ${data.numeroBackOrder}</title>
    <style>
    ...
    <div class="a4-page">
        <header class="header">
            <div class="brand-section">
                ...
                <div class="doc-title">
                    <h1>Back Order</h1>
                    <p class="doc-ref">${data.numeroBackOrder}</p>
                </div>
            </div>
        ...
        <footer class="footer">
            <div class="terms">
                ...
                <div class="terms-content">
                    ...
                    <p class="terms-text">${data.notes || 'Backordered quantities will be fulfilled and delivered as soon as stock becomes available.'}</p>
                </div>
            </div>

            <div class="totals-box">
                <div class="subtotal-row">
                    <span>Reference Delivery Note</span>
                    <span>${data.numeroBonLivraison || '---'}</span>
                </div>
                ...
                <div class="grand-total-card">
                    ...
                    <div class="total-amount">${new Intl.NumberFormat('en-GB').format(totalPending)}</div>
                </div>
            </div>
        </footer>
    </div>
