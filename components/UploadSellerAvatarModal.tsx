...
import { ApiError, MediaService, SellerAdminService } from "@/src/src2/api";
...
const UploadSellerAvatarModal = ({ isOpen, onClose, sellerId, username, profileImageUrl, onUploaded }: Props) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await Promise.all([
        ...
        SellerAdminService.updatePhoto(sellerId, { profileImageUrl: photoUrl }),
      ]);
      ...
      <div className="relative w-full max-w-lg bg-white shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right duration-300">
        ...
        <div className="px-8 py-4 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-secondary uppercase tracking-tight">Profile Photo</h2>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-5 bg-gray-50/50">
          ...
          >
            {previewUrl ? (
              ...
              <img src={previewUrl} alt="Preview" className="w-32 h-32 rounded-full object-cover border border-secondary-light" />
            ) : (
              <SellerAvatar name={username} imageUrl={profileImageUrl} size={128} />
            )}
...
export default UploadSellerAvatarModal;
