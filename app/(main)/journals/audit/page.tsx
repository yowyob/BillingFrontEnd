...
import { FactureFournisseurResponse, FactureFournisseurControllerService, AgenciesService, SellerAdminService, KernelAgencyResponse, SellerListItemResponse } from '@/src/src2/api';
...
const SupplierInvoiceJournalPage = () => {
  const [data, setData] = useState<FactureFournisseurResponse[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      FactureFournisseurControllerService.getByOrganizationId3(seller.organizationId),
      ...
      .catch(() => toast.error("Failed to load supplier invoice journal data."))
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      const matchesSupplier = !filters.supplier || (item.nomFournisseur ?? '').toLowerCase().includes(filters.supplier.toLowerCase());
      const itemDate = new Date(item.dateFacture ?? 0);
  ...
  const handleExportCsv = () => {
    exportRowsToCsv(
      `supplier-invoice-journal-${new Date().toISOString().slice(0, 10)}`,
      [
        { header: "Invoice Number", accessor: (r: FactureFournisseurResponse) => r.numeroFacture },
        { header: "Supplier", accessor: (r: FactureFournisseurResponse) => r.nomFournisseur },
        { header: "Agency", accessor: (r: FactureFournisseurResponse) => (r.agencyId ? agencyById.get(r.agencyId)?.name : undefined) },
        { header: "Date", accessor: (r: FactureFournisseurResponse) => r.dateFacture },
        { header: "Status", accessor: (r: FactureFournisseurResponse) => r.statut },
        { header: "Total (TTC)", accessor: (r: FactureFournisseurResponse) => r.montantTTC },
        { header: "Resting", accessor: (r: FactureFournisseurResponse) => r.montantRestant },
        { header: "Currency", accessor: (r: FactureFournisseurResponse) => r.devise },
      ],
      filteredData
    );
  };
  ...
  const getStatusColor = (statut?: FactureFournisseurResponse.statut) => {
    switch (statut) {
      case FactureFournisseurResponse.statut.PAYE: return "bg-emerald-50 text-emerald-600 border-emerald-200";
      case FactureFournisseurResponse.statut.BROUILLON: return "bg-secondary-super-light text-secondary-mid border-secondary-light";
      case FactureFournisseurResponse.statut.EN_RETARD: return "bg-red-50 text-red-600 border-red-200";
      case FactureFournisseurResponse.statut.PARTIELLEMENT_PAYE: return "bg-amber-50 text-amber-600 border-amber-200";
      case FactureFournisseurResponse.statut.ANNULE: return "bg-gray-100 text-gray-500 border-gray-200";
    ...
    <div className="p-8 bg-secondary-background min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <h1 className="text-3xl font-black text-primary tracking-tight">Supplier Invoice Journal</h1>
          <p className="text-secondary-gray text-sm font-medium">Monitoring organizational payables across agencies.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <div className="relative">
            <select className="w-full px-4 py-2.5 bg-secondary-super-light border border-secondary-light/50 rounded-xl text-sm font-black text-secondary-mid appearance-none text-center cursor-pointer hover:bg-secondary-mid hover:text-white transition-all outline-none" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}>
              ...
              {Object.values(FactureFournisseurResponse.statut).map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        </div>
              ...
              {loading ? (
                ...
                <tr><td colSpan={5} className="p-24 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-sm font-black text-secondary-gray uppercase tracking-widest">No supplier invoices found</span>
                  </div>
                </td></tr>
                ...
                return (
                  <tr key={item.idFactureFournisseur} className="hover:bg-secondary-super-light/40 transition-all group relative border-l-4 border-l-transparent hover:border-l-secondary-mid">
                    <td className="px-8 py-5">
                      <div className="flex flex-col">
                        <span className="font-black text-primary group-hover:text-secondary-mid transition-colors">{item.numeroFacture}</span>
                        <span className="text-[11px] font-bold text-secondary-gray flex items-center gap-1 mt-0.5"><Calendar size={10} /> {new Date(item.dateFacture ?? 0).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2 text-sm font-bold text-primary uppercase tracking-tight"><User size={14} className="text-secondary-mid" /> {item.nomFournisseur}</div>
                        <div className="flex items-center gap-3">
...
export default SupplierInvoiceJournalPage;
