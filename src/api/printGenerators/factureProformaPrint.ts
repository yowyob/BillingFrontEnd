// Mirrors facturePrint.ts's exact A4 layout/CSS, adapted for
// ProformaInvoiceResponse fields. A proforma has no balance-due/paid state
// (it isn't a settled invoice), so the balance block from facturePrint.ts
// is replaced with the offer validity window instead.
...
export const generateFactureProformaHTML = (
  data: any,
  org: OrgBranding,
  qrBase64: string
): string => {
  ...
  const tableRows = (data.lignesFactureProforma ?? []).map((item: any) => `
    <tr>
...
<head>
    <meta charset="UTF-8">
    <title>Proforma Invoice - ${data.numeroProformaInvoice}</title>
    <style>
    ...
    <div class="a4-page">
        <header class="header">
            <div class="brand-section">
                ...
                <div class="doc-title">
                    ...
                    <p class="doc-ref">${data.numeroProformaInvoice}</p>
                </div>
            </div>
        ...
        <section class="info-grid">
            ...
            <div class="date-box">
                ...
                <div class="date-row">
                    <span class="date-label">Valid For</span>
                    <span class="date-value" style="color: #2563eb;">${data.validiteOffreJours ? data.validiteOffreJours + ' days' : '---'}</span>
                </div>
            </div>
        </section>
        ...
        <footer class="footer">
            <div class="terms">
                ...
                <div class="terms-content">
                    ...
                    <p class="terms-text">${data.conditionsPaiement || 'This proforma invoice is for information purposes only and is not a demand for payment. Digital approval via the QR portal is legally binding.'}</p>
                </div>
            </div>
            ...
            <div class="totals-box">
                ...
                <div class="balance-row balance-paid">
                    <span>${data.statut === 'ACCEPTE' ? 'Accepted' : 'Not Yet Invoiced'}</span>
                    <span></span>
                </div>
            </div>
        </footer>
