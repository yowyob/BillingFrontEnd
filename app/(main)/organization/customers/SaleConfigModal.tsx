...
import { ApiError, ClientResponse, ThirdPartySaleConfigService } from "@/src/src2/api";
...
const SaleConfigModal = ({ isOpen, onClose, customer }: Props) => {
  ...
  useEffect(() => {
    if (!isOpen || !customer?.idClient) return;
    document.body.style.overflow = "hidden";

    setIsLoading(true);
    ThirdPartySaleConfigService.get(customer.idClient)
      .then((res) => {
        setSaleSizes(new Set(res.allowedSaleSizes ?? []));
        setVatApplicable(res.vatApplicable ?? false);
      })
      .catch(() => {
        setSaleSizes(new Set(customer.allowedSaleSizes ?? []));
        setVatApplicable(customer.ntva ?? false);
      })
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await ThirdPartySaleConfigService.set(customer.idClient, {
            ...
            <>
              ...
              <div>
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-100">
                  <label className={`${checkboxRow} cursor-pointer`}>
                    <input
                      type="checkbox"
                      className={checkboxStyle}
                    ...
                    />
                    <span className="text-sm text-gray-600 font-medium">VAT Applicable</span>
                  </label>
                </div>
              </div>
            </>
