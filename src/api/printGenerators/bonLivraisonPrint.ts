...
// BonLivraisonResponse (delivery note) fields. LigneBonLivraison only
// carries a single quantite (no ordered-vs-delivered split), so the line
// items table shows quantity + amount rather than the Qty/Unit/Total
// invoice-style breakdown, and the totals block is a simplified summary.
...
export const generateBonLivraisonHTML = (
  data: any,
  org: OrgBranding,
  qrBase64: string
): string => {
...
<head>
    <meta charset="UTF-8">
    <title>Delivery Note - ${data.numeroBonLivraison}</title>
    <style>
        ...
        * { box-sizing: border-box; -webkit-print-color-adjust: exact; }
        ...
        .balance-row { display: flex; justify-content: space-between; padding: 12px 16px; margin-top: 8px; font-size: 11px; font-weight: 900; text-transform: uppercase; border-radius: 12px; }
        .balance-paid { background-color: #ecfdf5; color: #059669; }
    ...
    <div class="a4-page">
        <header class="header">
            <div class="brand-section">
                ...
                <div class="doc-title">
                    <h1>Delivery Note</h1>
                    <p class="doc-ref">${data.numeroBonLivraison}</p>
                </div>
            </div>
        ...
        <section class="info-grid">
            ...
            <div class="date-box">
                <div class="date-row">
                    ...
                    <span class="date-value">${formatDate(data.dateLivraison)}</span>
                </div>
                <div class="date-row">
                    <span class="date-label">Sales Order Ref</span>
                    <span class="date-value" style="color: #2563eb;">${data.saleOrderNumber || '---'}</span>
                </div>
            </div>
        </section>
        ...
        <footer class="footer">
            <div class="terms">
                ...
                <div class="terms-content">
                    <p class="section-label" style="border: none; padding: 0;">Delivery Notes</p>
                    <p class="terms-text">${data.notes || 'Please inspect goods upon receipt and report any discrepancies within 48 hours.'}</p>
                </div>
            </div>
            ...
            <div class="totals-box">
                ...
                ${data.partial ? `
                <div class="balance-row" style="background-color: #fef2f2; color: #dc2626;">
                    <span>Partial Delivery</span>
                    <span></span>
                </div>` : `
                <div class="balance-row balance-paid">
